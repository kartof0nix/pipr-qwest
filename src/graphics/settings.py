...
class SettingsItem(urwid.AttrMap):
    def __init__(self, item: str, widget: urwid.Widget, title: str = None):
        ...
        super().__init__(urwid.Columns([('pack', self.left), ('pack', dec), (
            'pack', self.right), ('pack', inc),], focus_column=1), None, focus_map="reversed")
    ...
    def subpage(self, s: Setting):
        ...
        for item in s.config:
            if (s.constrains[item]['value_type'] == "selectable"):
                self.body.append(SettingsItem(item, SelectBox(
                    s.constrains[item]['selectable'], s.config[item])))
            if (s.constrains[item]['value_type'] == "bound"):
                self.body.append(SettingsItem(item, ValueBox(
                    s.constrains[item]['min_val'], s.constrains[item]['max_val'], s.config[item])))
        apply_bt = buttonAttr(urwid.Button("Apply", self.apply))
        exit_bt = buttonAttr(urwid.Button("Close", self.exit))
        footer = [urwid.Divider(), urwid.Columns(
            [("pack", apply_bt), ("pack", exit_bt)], dividechars=2)]
