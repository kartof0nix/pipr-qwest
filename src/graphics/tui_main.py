...
class blockLineBock(urwid.LineBox):
    def keypress(self, size, key):
        key = super().keypress(size, key)
        if (key is not None and key not in ['up', 'down', 'left', 'right', 'i']):
            return key


class MainView(urwid.WidgetPlaceholder):
    def is_overlayed(self):
        return isinstance(self.main_placeholder.original_widget, myOverlay)
    ...
    def __init__(self):
        self.bottom_placeholder = urwid.WidgetPlaceholder(
            urwid.SolidFill(u' + '))
        self.main_placeholder = urwid.WidgetPlaceholder(
...
def add_frame(
    ...
    if (isinstance(side, tuple)):
        ...
        halign = {'left': 'left', 'top': 'center',
                  'right': 'right', 'bottom': 'center'}[side]
        valign = {'left': 'middle', 'top': 'top',
                  'right': 'middle', 'bottom': 'bottom'}[side]
    if not block_move:
    ...
    view.push_top(line_box, width=width + 2 if isinstance(width, int) else width,
                  height=height + 2 if isinstance(height, int) else height, halign=halign, valign=valign)
...
def render(callback, exitFunction):
    ...
    urwid.WidgetPlaceholder(null_attr)
...
async def test_view():
    ...
    add_frame(urwid.Text(
        "This is some text inside a fixed-size LineBox."), 6, 10, 'left', 'tet')
    logger.info("Created widget 1")
    await asyncio.sleep(4)
    add_frame(urwid.Text("You're bad at programming"), 40, 2,
              'bottom', "You like kissing boys, don't you?")
