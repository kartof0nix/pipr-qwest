...
class overlayWidget(urwid.Pile):
    def update(self):
        self.widgetList[0].set_text(
            [('hred', "Health:  "), str(player.player['health'])]),
        self.widgetList[1].set_text(
            [('hgreen', "Attack:  "), str(player.player.calcAttack())]),
        self.widgetList[2].set_text(
            [('hblue', "Defense:  "), str(player.player.calcDefense())])
