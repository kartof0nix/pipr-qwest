...
class dynamicTexture(texture):
    ...
    async def move_loop(self):
        while (True):
            ...
            while (time() < end):
                (dx, dy) = (
                    self.next_pos[0] - self.prev_pos[0], self.next_pos[1] - self.prev_pos[1])
                prog = min((time() - start) / cfg["anim speed"], 1)
                self.pos = (
                    int(self.prev_pos[0] + dx * prog), int(self.prev_pos[1] + dy * prog))
                urwid.CanvasCache.clear()
                tui_main.loop.draw_screen()
...
class PlayerTexture(dynamicTexture):
    ...
    def update(self, grid_size: Tuple[int, int], instant: bool = False):
        try:
            ...
            logger.error('Update dailed : %s, to field:%s ',
                         e, player.player['currentField'])
...
class itemSquare(item):
    ...
    def cords_to_pos(self, item_size: Tuple[int, int], cord: Tuple[int, int]) -> Tuple[Tuple[int, int], Tuple[int, int]]:
        def calc(n: int, poz: int):
            return int((n // 3) * poz + min(n % 3, poz))
    ...
    def sketch(self, item_size: Tuple[int, int]):
        if (self.cache is not None and self.cacheSize == item_size):
            return self.cache
    ...
    def _sketch(self, item_size: Tuple[int, int]):
        ...
        if (self.field.blocked):
            out = [[self.blocked if self.grid[i * 3 // x][j * 3 // y]
                    else self.bg for j in range(y)] for i in range(x)]
            style = [[self.blocked_style if self.grid[i * 3 // x][j * 3 // y]
                      else self.bg_style for j in range(y)] for i in range(x)]
        else:
            out = [[self.fg if self.grid[i * 3 // x][j * 3 // y]
                    else self.bg for j in range(y)] for i in range(x)]
            style = [[self.fg_style if self.grid[i * 3 // x][j * 3 // y]
                      else self.bg_style for j in range(y)] for i in range(x)]
        ...
        for it in self.decorations:
            ...
            if (not self.is_empty(poz, (dec_x, dec_y), out)):
                logger.info("Field %s could not draw decoration %s",
                            self.field.num, dec)
...
class itemSquareForest(itemSquare):
    def __init__(self, field):
        ...
        self.seed = self.field.num + \
            int(bytes(player.player['currentField']).hex(), 16)
    ...
    def _sketch(self, item_size):
        ...
        max_deco = (6 * max_size[0] * max_size[1]
                    ) // int((dec_size[0] * dec_size[1]) * (1.2))
