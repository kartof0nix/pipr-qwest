...
class fabricGrid(fabric):
    ...
    def init(self, level: Level, handlekey):
        ...
        self.lastSize = (0, 0)
        # Can't init player.player texture coz canva
        self.playerTexture = PlayerTexture(level, (0, 0))
        self.n = level.height
        self.m = level.width
        self.level = level
        self.grid = []
    ...
    def keypress(self, size, key):
        if (key == "esc"):
            tui_main.add_frame(PauseMenu(), ('relative', 80),
                               ('relative', 70), ('center', 'middle'), "Pause menu")
        elif (self.complete()):
...
class LevelView:
    ...
    def __enter__(self):
        try:
            ...
            tui_main.add_frame(overlay, width=20,
                               height='pack', side=('left', 'top'))
